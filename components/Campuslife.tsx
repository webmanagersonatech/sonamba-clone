...
export default function CampusLife() {
  return (
    <section className="px-6 lg:px-20  pb-16 bg-gradient-to-b from-white to-gray-100">
      {/* Heading */}
      <motion.h2
      ...
      {/* Image Grid */}
      <div className="grid  sm:grid-cols-2 lg:grid-cols-4 max-w-7xl mx-auto">
        {campusLife.highlights.map((item, index) => (
          <motion.div
            ...
            viewport={{ once: true }}
            className="relative group overflow-hidden shadow-md hover:shadow-2xl transition-shadow duration-500"
          >
            <Link href={item.href || "#" as any} className="block w-full h-full">
              <motion.div
                whileHover={{ scale: 1.03 }}
                transition={{ duration: 0.4, ease: "easeInOut" }}
                className="relative w-full h-64 sm:h-72 lg:h-60 overflow-hidden "
              >
                <Image
                  src={item.src}
                  alt={item.desc}
